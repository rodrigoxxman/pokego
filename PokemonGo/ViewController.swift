...
//  PokemonGo
//
//  Created by Rodrigo Salas J on 21/11/18.
        ...
        Timer.scheduledTimer(withTimeInterval: 1, repeats: false, block: { (timer) in
            if let coord = self.ubicacion.location?.coordinate {
                ...
                if MKMapRectContainsPoint(mapView.visibleMapRect, MKMapPointForCoordinate(coord)){
                    ...
                    let alertaVC = UIAlertController(title: "Felicidades!", message:"Atrapaste a un fuerte \(pokemon.nombre)", preferredStyle: .alert)
                    let pokedexAccion = UIAlertAction(title: "Pokedex", style: .default, handler:{
                        (action) in self.performSegue(withIdentifier: "pokedexSegue", sender: nil)
                    })
    ...
    func setup(){
        ...
        Timer.scheduledTimer(withTimeInterval: 5, repeats: true, block: { ( timer ) in
            if let coord = self.ubicacion.location?.coordinate{
                ...
                let randomLat = (Double(arc4random_uniform(200))-100.0)/10000.0
                let randomLon = (Double(arc4random_uniform(200))-100.0)/10000.0
